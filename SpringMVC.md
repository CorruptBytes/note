...
Cookie 是浏览器提供的一种轻量级键值数据存储机制，用于在无状态的 HTTP 协议之上实现会话关联。它可以在浏览器中保存少量键值对信息，并在符合作用域规则时自动随 HTTP 请求发送到服务端。浏览器向服务端发送Http请求后，服务器可以进行`Cookie`设置，也就是添加Http请求头中的`Set-Cookie`字段。Cookie以键值对的形式保存数据。服务端响应浏览器后，浏览器就会将`Cookie`的内容保存起来，此后浏览器向这个服务端发送的每一个请求都会自动携带这个`Cookie`信息。
