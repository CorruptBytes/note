...
- **至少一次投递**：MQ默认保证消息至少会被消费者成功消费一次，但可能存在重复消费。
...
消息传递语义定义了一条消息在传递链路中是否允许丢失、是否允许重复以及最终被处理次数的规则。

- 消息传递语义通常需要生产者，MQ，消费者三方共同配合实现。
- 

常见有三种消息传递语义：

<h3> <code>At Most Once</code>（最多一次）</h3>

消息最多被处理一次，可能丢，但不会重复

<h3><code> At Least Once</code>（最少一次）</h3>

消息至少被处理一次，可能重复，但不会丢

<h3><code>Exactly Once</code>（恰好一次）</h3>
...
集群模式下负载均衡，一个消费者组中的所有消费者共享消费进度，一个`MessageQueue`只能关联同一消费者组中的一个消费者，也就是说同一时刻一个 `MessageQueue` 只能被同一消费者组中的一个消费者消费，每一个消费者组在 `Broker` 端维护整个消费者组共享的消费进度；

广播模式下消息以广播方式分发，每个消费者都会独立消费该 `Topic `下的所有 `Queue`，各自在本地独立维护一份消费进度。
...
与`RabbitMQ`类似,当接受消息的`topic`不存在时，`RocketMQ`支持自动使用默认参数创建该`topic`。
...
- 创建出的`topic`队列数固定为4。
- 因为创建出的`topic`不可控，通常会关闭此选项。
- `RabbitMQ`可以由客户端驱动队列与交换机的创建；而`RocketMQ`将`Topic`视为一种集群资源，最好在`Broker`端自行配置，而不是由客户端驱动。
...
<h3>生产者(<code>RocketMQTemplate</code>)</h3>

`RocketMQ`提供了生产者的统一模板类`RocketMQTemplate`，支持发送各种类型的消息。

- 引入起步依赖后,会根据配置自动注入一个`RocketMQTemplate`。

<h3>消费者</h3>

`RocketMQ`支持通过添加类注解的方式配置一个消费者，消费者类需要实现`RocketMQListener`接口，此接口定义了消息消费方法。
...
- `RocketMQMessageListener`会在`onMessage`执行完成后再对消息进行`ACK`，如果`onMessage`因为抛出异常而执行失败，则不会`ACK`。

# `Kafka`

`Kafka`是由`LinkedIn`开发的分布式事件流平台，后捐赠给`Apache`基金会成为其顶级项目。`Kafaka`最初用于构建高吞吐的分布式日志系统，解决大规模日志收集与实时数据流传输问题。
...
对`Topic`的物理拆分，一个`Topic`可以在物理上拆分成多个`Partition`，不同的`Partition`可以在不同的服务器上，这增强了`Kafka`的并行能力，为`Kafka`提供了水平扩展能力。每个`Partition`都是一个有序日志。

- 一个消费者可以消费多个分区，但一个分区不能同时被同一个消费者组中的多个消费者消费。这是因为同一个消费者组中的消费者是共享消费进度的，如果同一个分区被同一个消费者组中的多个消费者消费，就需要引入锁等同步机制保证并发并发安全，会造成额外开销和并发竞争
- 不同消费者组中的消费者消费同一个分区时互相独立，互不影响

<h4><code>Offset</code>(偏移量)</h4>

当消息存储到分区后，`Kafka`会记录其偏移量，这是每条消息在 `Partition` 中的唯一编号,`Consumer`通过`Offset`记录消费进度。
...
<h4><code>Consumer Group</code></h4>

`Kafka`中多个消费者可以组成消费者组，同一个消费者组中的消费者会共同消费一个或多个主题中的消息。每条消息只能被同一消费者组中的一个消费者消费，但是不同消费者组是相互独立，互不影响的，可以重复消费同一条消息。

## 架构

<h3><code>ZooKeeper</code>架构</h3>

`Kafka`使用单独部署的`ZooKeeper`管理`Kafka`集群，包括`Broker`注册,`Controller`选举，元数据保存等。

![ed03b6b4e0744bc89a436dcf4e1b3344](D:\笔记\图片\ed03b6b4e0744bc89a436dcf4e1b3344.png)

<h3><code>KRaft</code>架构</h3>

从`Kafka`3.X开始，逐渐废弃`ZooKeeper`，转而使用`Raft`协议管理集群。从`Kafka 3.5`开始，完全废弃`ZooKeeper`，只能使用`KRaft`模式。

## `Broker`

Broker 是 Kafka 集群中的一个服务器节点，负责存储 Partition 数据、处理读写请求，并通过副本机制保证数据的高可用。

- 一个 Broker 可以管理多个 Partition；对于同一个 Partition，其 Leader 和 Follower 副本通常会分布在不同的 Broker 上，以实现高可用和容错。

## 部署

`Kafka`需要依赖本地`Java`运行时环境。

<h3>安装包部署</h3>

`Kafka`提供了源码安装包和二进制安装包，源码安装包需要本地编译后才能使用

1. 生成集群UUID

## `Replication`(副本机制)

`Replication`是`Kafaka`为了保证分区数据可靠性提供的功能，本质是一个`Partition`主从集群。每个`Partition`可以有多个副本(通过`replication-factor`设置)，分为`Leader`和`Follower`。其中，`Leader`负责所有读写操作，`Follow`仅从`Leader`中同步数据，当`Leader`异常后会从`Follower`中选一个作为`Leader`。
