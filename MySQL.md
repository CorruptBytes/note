...
- 各数据库管理系统在实现SQL标准操作的基础之上，还会引入了各自的方言（Dialect）与扩展能力，以优化性能或添加高级功能，因此一条SQL语句不一定在所有数据库管理系统中都能运行。
...
|            `BINARY(n)`            |  n字节   |             n 字节原始二进制数据(byte数组)             | 存放固定长度的数据 |      UUID      |
...
通过更严格的锁和MVCC保证多个并发事务执行的结果等价于某种串行执行顺序。不会发生脏读，不可重复读和幻读现象。
...
|            分类            |                            含义                             |        特点         |
| :------------------------: | :---------------------------------------------------------: | :-----------------: |
| 聚簇索引(Clustered  Index) |   将数据与索引一起存储，数据行本身存储在 B+ 树叶子节点中    | 必须有,而且只有一个 |
| 二级索引(Secondary  Index) | 将数据与索引分开存储，B+ 树叶子节点只存储索引列与对应主键值 |    可以存在多个     |

- 二级索引的排序规则为：如果索引值不同，则从小到大排序；如果索引值相同，按照主键值从小到大排
...
**二级索引只存储索引列和主键值；如果查询还需要其他列，MySQL 会先通过二级索引查出主键，然后回聚簇索引取值，这个过程称为回表查询。**
...
|                    |                                                  |
| :----------------: | :----------------------------------------------: |
|      单列索引      | 建立在单列上的索引，索引中会存储主键和单列字段值 |
| 联合索引(复合索引) | 建立在多列上的索引，索引中会存储主键和多列字段值 |

- 联合索引使用多个字段作为`B+Tree`的`key`进行排序，优先按照第一个字段排序，第一个字段相同时按照后续字段排序。
...
执行计划由优化器生成，它决定了 SQL 语句的执行过程，例如使用哪个索引、表的访问类型以及连接方式等。
...
分表指将单张表按照规则拆分成多张表，每个表只存储一部分数据。

分表主要为了解决单表数据量过大，导致查询性能下降的问题，且对该表进行`DDL`操作时，会导致整张表被长时间锁住，业务停摆。

一般数据达到千万级别，就需要考虑分表。因为此时`B+`树索引高度一般会达到三层及以上，查询时需要更多的`IO`次数。
...
分库是一种水平扩展数据库的技术，将数据根据一定规则拆分到多个独立的数据库实例中，每个数据库只存储部分数据，实现了数据分片和分布式存储。

分库主要为了解决单机的性能瓶颈问题：

- 单机磁盘存储存在上限，将数据存储在不同数据库实例中，相当于水平扩展磁盘容量。

- 数据库连接数有限，并发连接过多，单数据库实例无法满足。
...
分片键，即用来**分库/分表**的字段，决定数据路由规则。分片键的选择需要在**数据分布均衡**和**查询局部性**之间取得平衡

一方面，要让“强关联数据”落在同一分片，以避免跨分片事务或高频查询需要扫描所有分片(全路由)；另一方面，要保证数据均匀，避免数据倾斜和写入倾斜导致单点压力过大。

此外，分片键应该保证稳定，应选择业务长期稳定字段，一旦选定，就不能修改，因为修改需要全量数据迁移。

<h4>非分片键条件查询如何查询</h4>

分库分表后，无法避免存在一些业务场景，需要通过非分片键构建复杂查询，这一类查询往往需要涉及多表甚至多库，通常有以下方案实现：

- **遍历：**最简单粗暴的方法，遍历所有表，找出符合条件的数据。这会导致全路由，多库聚合，性能差。

- **搜索中间件：**引入`ES`等适合复杂搜索的中间件，将数据冗余同步到`ES`中，通过`ES`进行复杂查询。如果有更新操作，可以通过`MQ`等手段异步同步到`ES`。

  由于ES不是强一致性，可能存在片刻的数据不一致问题，这可以通过查询降级来解决。如果ES查询不到结果，则降级到`MYSQL`中进行查询。

- **基因法：**将分片键编码进非分片键中，业务可以解析非分片键得到分片键，从而快速定位数据位于哪一分片。

- **异构索引表：**本质是空间换时间，异构索引表是为了满足不同查询需求，而额外建立的、数据组织方式不同的索引型冗余表。它可以不存储完整数据信息，仅仅是作为索引表辅助快速定位主表数据位置。

  - 异构索引表可以采用`MQ`异步同步数据的方式以维护与主表的数据一致性，并通过事务消息或本地事务表 + 定时轮询保证索引表与主表的一致性。并添加兜底重试机制(配合定时任务扫描未确认的消息，确保索引表不会丢失数据)

<h4>分片策略</h4>

**数据范围**

按照数据的范围进行分片，如主键范围，时间范围
...
按照分片键取模或`hash`后取模的值对数据进行分片

- 不存在热点问题
- 不便于扩容，扩容可能需要全量数据迁移，最好一次性规划好，避免二次扩容。
...
对于超大数据级别，可以结合多种策略进行多次分片，如先根据数据范围进行分库，再在每个库中通过`hash`取模分表。既可以在一定程度上避免数据倾斜和热点问题，也更便于扩容。

<h4>跨节点连表查询</h4>

当分库后，可能相互关联的表会位于不同的数据库实例中，此时无法直接使用`join`或子查询进行连表查询，通常可以使用以下方案替代：

- **字段冗余存储：**在主表中冗余存储相关的字段，避免连表查询
- **应用层组装：**进行多次查询，然后在应用层组装数据
- **数据聚集：**使用`ETL`工具，定期对相关联的表中数据进行同步与聚合，生成新的表。

<h4>分页问题</h4>

分库分表后，分页数据可能存在在不同的节点中。

- **全局视野法：**在各个节点查询出足够的数据后，在应用层进行排序汇总。这种方案在深分页中会导致内存和网络IO爆炸，必须限制最大页数
- **游标分页：**记录上一页的最大排序值，查询下一页时，根据上一页最大排序值查询出每个节点的下一页数据，最后汇总排序
- 使用`ES`等更加合适的搜索引擎
...
1. **引入代理层：**在数据访问层之上增加一层**路由代理**，并引入开关控制访问策略。
2. **全量发布后开启双写：**在新版本全量发布后开启双写机制，使所有写请求同时写入旧库和新库，以保证增量数据的一致性。同时记录一个迁移分界点，例如时间戳或自增ID，用于区分存量数据和新增数据，确保迁移过程可以有明确边界。
3. **存量数据回填：**通过离线任务将分界点之前的历史数据从旧库批量迁移到新库。这一过程通常采用分批执行的方式，以避免对线上数据库造成压力，同时通过控制速率和校验机制保证数据完整性与正确性。
4. **切换读流量并验证一致性：**在存量数据回填完成后，将读请求逐步从旧库切换到新库，可以采用灰度方式逐步放量。在此阶段写操作仍然保持双写，从而保证仍可能存在的边界数据不会丢失，同时观察新库的查询正确性与系统稳定性。
5. **停止旧库写入并完成迁移：**当新库稳定运行一段时间且业务指标正常后，可以停止旧库写入，将系统完全切换为新库读写模式。之后逐步下线旧库，实现整个分库分表扩容过程的闭环。
...
乐观锁的核心思想是不对数据加锁，而是在进行更新时检查资源是否被其他线程修改。如果被修改，则放弃更新，可以抛出异常或自旋重试；如果没有被修改，则可以安全的进行更新。

在MYSQL中，可以通过为行记录添加版本号(`version`)或时间戳(`timestamp`)实现乐观锁。在执行更新前检查版本号或时间戳是否变更。如果未变更，则执行更新操作并更新版本号或时间戳；如果变更，则放弃修改，可以在应用层自旋重试或抛出异常。
