...
- **修饰变量：**表示该变量只能赋值一次，不能再对其进行重新赋值操作，否则编译时会报错(但允许延迟赋值)
...
`throws`主要用于受检异常的的处理责任传递，声明当前方法可能抛出此编译时异常，但本方法不处理，抛出由上层处理。
...
- 编译时异常必须进行`try-catch`或`throws`后由上层`try-catch`，否则编译报错。
- `throws`用于运行时异常时，只是在语义上向调用者提示这里可能有运行时异常，不会改变编译器的异常处理规则和运行时异常传播行为，与不写效果相同。

# 泛型

泛型是JDK5中引入的特性，是 **一种在定义类、接口或方法时，不指定具体数据类型，而在使用时再指定的机制**。编译器会在编译阶段约束操作的数据类型，并进行检查。

- 本质上，泛型就是一种编译时类型检查机制。

```java
//使用
数据类型<引用数据类型> = new 数据类型<引用数据类型>();

//菱形语法：编译器可以根据上下文直接推断出 new 对象的泛型类型，从而无需在 new 时显示声明泛型类型
...
通配符可以用在泛型的使用位置上，如方法参数、变量类型上，用来约束这个引用能接受什么泛型范围的实例，但不能用于泛型定义处。

```java
//无界通配符 <?> 可以接收任何泛型 List 不允许往里面写（除了 null）
List<?> list;
//上界通配符 <? extends T>
...
```
Producer → extends（生产数据） 只能读，不能写
Consumer → super（消费数据） 可以写，但读时不安全
```
...
桥接方法（`bridge method`）是 Java 编译器在**泛型擦除（type erasure）之后自动生成的方法**，用来保证**多态在泛型场景下仍然成立**。

- 本质是编译器生成的转发方法，用来连接泛型擦除前后的方法签名差异。
...
由于泛型擦除，编译后子类与父类的方法签名可能不再一致，导致 JVM 无法正确识别方法重写，导致多态失效。
...
此时编译器会生成一个返回父类类型的方法并内部转发到真实实现，从而保证多态仍然成立。
...
底层使用数组来存储键值对，键值对在数组中的位置通过键值对中`key`的`hashcode % (table.length -1)`来决定，相同位置的键值对通过链表连接以解决冲突。
...
从查询复杂度上看，链表为O(n)，红黑树为O(log n)。在 n 较小时，log n 的优势并不明显，且考虑红黑树较高的维护成本和更高的内存占用，在小规模数据下，链表的实际性能往往优于红黑树。
...
`LRU`缓存基于访问时间对数据进行淘汰。当需要淘汰数据时，淘汰最久没有被访问到的数据。
