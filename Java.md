...
基本数据类型是是预定义的，不需要实例化就可以使用。Java中的基本数据类型有八种，大致可分为数值型，字符型，布尔型。
...
- 若实际类型于目标类型不兼容会抛出`ClassCastException`，为保证转换安全，可以使用`instanceof`关键字检查实际类型，检查通过再进行转换
...
**Java14中，instanceof增加了新特性，可以在判断的同时直接定义变量，减少显式强制类型转换**
...
- **`Exception`**表示程序可能出现的问题，`Exception`的子类又可分为两类：
  ...
  - `RuntimeException`外的其他异常被称为编译时异常，在编译阶段需要对其显示的处理。如果不进行处理，则编译失败
...
底层使用数组来存储键值对，键值对在数组中的位置通过键值对中`key`的`hashcode`来决定，又称为哈希桶。

- HashMap支持`null`键和值，如果键为`null`,则其`hashCode`为0

**哈希冲突**

> 当不同的元素计算出的`hashCode`一致时，称为哈希冲突。

- 在`HashMap`中，当`hashCode`一致时，会通过链表或红黑树来解决哈希冲突。
...
在 Java 7 及以前，HashMap 的底层数据结构是 **数组 + 链表**。HashMap 通过 Entry 数组存储键值对，每个 Entry 节点包含一个 `next` 指针，用于将发生哈希冲突的多个 Entry 串联成链表。当多个 key 经过 `hashCode` 计算后定位到同一个数组下标，并且通过 `equals` 比较 key 不相等时，这些 Entry 就会通过 `next` 指针形成链表来解决哈希冲突。查询时，HashMap 先根据 key 的 hash 定位到数组下标，再在对应链表中通过 `equals` 逐个比较 key，时间复杂度在冲突严重时可能退化为 O(n)。

在 Java 8 中，HashMap 在 **数组 + 链表** 的基础上引入了 **红黑树** 来优化冲突严重时的查询性能。默认情况下仍使用链表存储冲突节点；当某个桶中的链表长度 **≥ 8 且数组容量 ≥ 64** 时，链表会转换为红黑树，以将查询时间复杂度从 O(n) 优化为 O(log n)；当红黑树中节点数量 **≤ 6** 时，又会退化回链表，以降低红黑树维护成本。这种链表与红黑树的动态切换，在保证空间效率的同时显著提升了 HashMap 在高冲突场景下的性能。
