...
Python主要采用引用计数法进行垃圾判断，当引用计数变为 0时，立即释放对象。为了解决循环引用问题，`CPython`引入了分代循环垃圾回收机制。循环垃圾回收的核心思想是试探性删除引用和局部可达性分析，它会跟踪可能产生循环引用的对象(包括容器对象，自定义对象等)，在每次GC时，把这一代及其更年轻的代里**所有被跟踪的容器对象**一起拿出来，
