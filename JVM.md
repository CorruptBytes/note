...
Python主要采用引用计数法进行垃圾判断，当引用计数变为 0时，立即释放对象。为了解决循环引用问题，`CPython`引入了分代循环垃圾回器收器。
它会按照代为每一代维护一个链表，链表中存放了此代中会产生循环引用的对象(也就是容器对象，如list，dist，自定义对象等)。垃圾回收器周期性的扫描链表中的引用，构建对象之间的引用图，然后模拟删除内部引用。模拟删除后，引用计数大于0的对象说明存在外部入口，等于0的对象可能是循环垃圾。然后基于大于0的对象做可达性传播，最后所有不可达的对象就是循环垃圾

<h3>可达性分析法</h3>

可达性分析法将堆中的对象分为两类：垃圾回收的根对象(GC Root)和普通对象。`GC Root`对象与普通对象之间的引用关系可以构成一张有向图。从一组`GC Root`出发，沿着引用关系向外搜索，能访问到的对象就是可达的，不可以被回收；访问不到的对象就是不可达的，可以被回收。
- 使用可达性分析判断垃圾时，垃圾回收过程通常分为两个阶段：第一个阶段为标记阶段，使用可达性分析标记出所有存活对象。第二阶段为清除阶段，对未标记的对象进行回收清除。
...
<h4>三色标记法</h4>
三色标记法是可达性分析的一个具体工程实践，常用于在并发垃圾回收的并发标记阶段，用以解决边标记边运行程序时的数据一致性问题。
三色标记法使用三种颜色标记遍历过程中被遍历对象的状态：
- 白色为没有被访问到的对象。如果遍历完成对象仍为白色，说明该对象不可达，可以被回收。
- 灰色为已经被访问到的对象，但这个对象内部引用的其他对象没有被全部访问到。
- 黑色为已经被访问到的对象，且内部引用的其他对象也全部被访问到。
