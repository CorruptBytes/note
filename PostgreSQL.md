...
`PostgreSQL`采用堆表存储数据，因此其`MVCC`实现机制也与`MYSQL`有所不同。

**优点**

- 更新时追加一条记录而非覆盖原有记录，有利于减少高并发写操作的冲突。
- **数据本身就是历史：**在表中直接存储同一记录的多个版本，通过每个版本携带的`xmin`和`xmax`配合事务快照判断当前事务可以看到哪个版本，不依赖外部日志和版本回溯。

**缺点**

- 每次更新都会产生新行，且旧行不会立即删除。在高并发写场景下，会导致表膨胀。

------

`PostgreSQL`在建表时，除了用户字段外，还会添加多个隐藏字段：

- `xmin`：创建该行的事务ID
- `xmax`：删除或更新该行的事务ID

当插入新记录时，创建一个新行，`xmin`设置为当前事务ID，`xmax`设置为`NULL`。

当更新一条记录时，不会覆盖原数据，而是修改原有行记录的`xmax`为当前事务ID，并在表中插入一条新记录，`xmin`为当前事务ID。

当删除记录时，将改行的`xmax`设置为当前事务ID，不立即进行删除。

因此查找记录时，`PostgreSQL`会扫描目标记录的多个版本，然后进行可见性过滤，找到并返回对当前事务可见的版本，而不是通过版本回溯。

<h4>可见性判断规则</h4>

`PostgreSQL`通过记录的`xmin`/`xmax`字段与当前事务快照判断当前记录是否对于该事务可见。

`xmin`判断创建可见性，`xmax`判断删除/覆盖可见性

当事务开始时，会生成一个`snapshot`，它记录了：

- 当前最小活跃事务ID
- 下一个将要分配的事务ID
- 当前还没提交的事务ID集合

<h4>清理机制(<code>VACUUM</code>)</h4>

由于`PostgreSQL`在更新时会不断在表中产生旧版本，因此需要清理被所有事务都看不到的旧版本，这种机制称为`VACUUM`。
