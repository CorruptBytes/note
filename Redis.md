...
二级缓存通常由 Caffeine（本地缓存）+ Redis（分布式缓存）组成。

读流程是：优先查本地缓存，未命中再查 Redis，再未命中则查询数据库，并回填两级缓存。

写流程一般采用 Cache Aside 模式：先更新数据库，再删除 Redis 和本地缓存，而不是直接更新缓存，以避免并发导致的数据不一致问题。

对于本地缓存一致性问题，可以通过消息队列或 Redis Pub/Sub 通知各个实例删除本地缓存。

同时 Redis 需要配置 TTL、防止缓存雪崩，并结合布隆过滤器、互斥锁等手段解决缓存穿透和击穿问题。
