...
数据库中数据更新时，会出现缓存与数据库数据不一致的情况。缓存更新策略决定何时更新缓存数据，以保证缓存与数据库的数据一致性。

|          |                           内存淘汰                           |                           超时剔除                           |                   主动更新                   |
| :------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :------------------------------------------: |
|   说明   | 无需自己维护，利用Redis的内存淘汰机制，当内存不足时自动淘汰部分数据。下次查询时更新缓存 | 给缓存数据添加TTL时间，到期后自动删除缓存。下次查询时更新缓存。 | 编写业务逻辑，在修改数据库的同时，更新缓存。 |
...
**如何保证缓存与数据库操作同时成功或失败:**单体系统可以使用事务，分布式系统采用TCC等分布式事务方案。
...
查询的 **数据在缓存中不存在，同时在数据库中也不存在**，导致每次请求都会绕过缓存直接访问数据库。
...
- **布隆过滤器：**在缓存之前加入一个布隆过滤器，布隆过滤器会根据一种算法计算出该数据是否存在在数据库中，如果计算结果未不存在，则直接返回请求，不会进入缓存与数据库。
  ...
  **缺点**
  ...
  - 布隆过滤器存在误判可能，当他计算数据存在时，数据可能并不存在；不过他计算数据不存在时，数据一定不存在。
  - 实现复杂
...
当 Redis 执行写请求时，当前写入会导致内存达到或超过 `maxmemory`，Redis 会根据配置的内存淘汰策略删除部分 key 以释放内存，防止内存不足而崩溃。
...
> 缓存就是数据交换的缓冲区(英文为Cache),是临时存储数据的地方，一般读写性能较高
...
- 主动更新策略
  ...
  > 注意：
  >
  > - 最常用的为Cache Aside Pattern,这种方式需要自己编码，虽然麻烦，但可靠性更高
  > - 主动更新策略在修改缓存时，有两种方式:在修改数据库时直接删除缓存，下次查询到来时更新；或者每次更新数据库时同步更新缓存。应该选择直接删除缓存，因为多次更新缓存会导致许多无效的写操作
  > - 保证缓存与数据库操作的一致性，对于单体系统，可以使用事务，对于分布式系统，可以使用TCC等分布式事务方案
  > - 还会存在到底先修改数据库还是先删除缓存的抉择，考虑线程安全问题，应该先更新数据库，然后删除缓存，因为先更新数据库再删除缓存的情况下，出现线程安全问题的概率远远减小

![image-20250511191238155](D:\笔记\图片\image-20250511191238155.png)

![image-20250511191302828](D:\笔记\图片\image-20250511191302828.png)

### 缓存穿透

> 指客户端请求的数据在缓存中和数据库中都不存在，这时缓存永远不会生效，这些请求都会到达数据库

- 解决方案

  1. 缓存空对象

     > 在redis中缓存一个空对象，当请求再次到达时，会返回这个空对象，不会到达数据库
     ...
     > 布隆过滤是一种算法，但是仍然存在缓存穿透风险
  
     ![image-20250512105816189](D:\笔记\图片\image-20250512105816189.png)
...
> 在同一时间段大量的缓存key同时失效或者Redis服务宕机，导致大量请求到达数据库，给服务端带来大量压力

- 解决方案
  1. 给不同Key的TTL添加随机值(防止同一时间key大量失效)
  2. 利用Redis集群提高服务器的可用性
  3. 给缓存业务添加降级限流策略
  4. 给业务添加多级缓存

### 缓存击穿

> 又被称为热点key问题，一个被高并发访问并且缓存重建业务教复杂的key突然失效，无数的请求访问会在瞬间给数据库带来巨大冲击

- 解决方案

  1. 互斥锁

     ![image-20250512115033104](D:\笔记\图片\image-20250512115033104.png)

  2. 逻辑过期

     > 不为key添加ttl，在数据中使用一个字段声明其过期时间

![image-20250512115223192](D:\笔记\图片\image-20250512115223192.png)

|          |                  优点                  |                  缺点                  |
| :------: | :------------------------------------: | :------------------------------------: |
|  互斥锁  | 没有额外内存消耗，保证一致性，实现简单 |  线程需要等待，性能受影响，有死锁风险  |
...
- 如果没有从节点，那么可以选择在Redis业务压力的低峰阶段进行扫描查询，以免影响到正
  常处理业务；或者可以使用-i参数控制扫描间隔，避免长时间扫描降低Redis实例的性能。
