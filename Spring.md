...
通过依赖注入，类只需要声明自己所依赖的对象类型，IOC容器会在运行时将这些依赖实例注入到对象中，而无需在类内部通过`new`关键字创建依赖对象，使对象与依赖之间大幅度解耦。
...
又称为集合类型依赖注入。当字段类型为集合时，`Spring`在进行依赖注入时，会自动收集容器中所有符合泛型类型 的 `Bean`，并组装成集合注入。

- 在 Spring Framework 中，当注入集合类型时，默认执行多值依赖注入，会根据泛型类型收集容器中所有匹配的 Bean 并组装成集合。如果不存在符合类型的 Bean，则注入空集合。只有在使用 `@Qualifier` 或 `@Resource` 显式指定 `beanName` 时，才会按名称进行依赖查找，尝试注入已有的集合类型 `Bean`。
...
| `Map<String,T>` | `LinkedHashMap` |

- 可以使用`Order`接口或`@Order`注解定义`Bean`在集合中的顺序
- `Map`的`key`为`beanName`。
...
`Bean`是由`Spring`容器负责创建、管理和销毁的对象，本质就是一个被`IOC`容器托管的普通Java对象。

## 生命周期

一个 Bean 从被 Spring 容器创建开始，到被容器销毁为止，所经历的完整过程。

<h3>实例化</h3>

Spring通过反射创建`Bean`实例

- 通过反射调用构造方法，构造器依赖注入在这里执行

<h3>依赖注入</h3>

通过反射对添加了`@Autowired`,`@Resource`注解的`Setter`方法或字段进行依赖注入

<h3>初始化</h3>

初始化可分为三个步骤：①`BPP`前置处理②执行初始化方法③`BPP`后置处理。

<h4><code>BeanPostProcessor</code>前置处理</h4>

依次调用`BeanPostProcessor`对象的`postProcessBeforeInitialization()`对`Bean`进行初始化前增强。

<h4>初始化方法</h4>

Spring 调用 Bean 自己配置的初始化方法，初始化方法配置方式有（按执行顺序）：
...
<h4><code>BeanPostProcessor</code>后置处理</h4>

依次调用`BeanPostProcessor`对象的`postProcessAfterInitialization()`方法对`Bean`进行初始化后增强。

<h3>使用</h3>

Bean 已经完全创建完成，可以从`IOC`容器中获取使用。

<h3>销毁</h3>

当容器关闭时，Spring 会销毁由容器管理的 **单例 Bean**，调用 Bean 配置的销毁方法以释放资源和清理状态。
